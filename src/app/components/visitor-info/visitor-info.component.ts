...
export class VisitorInfoComponent implements OnInit {
  ...
  constructor(
    public sheetRef: MatBottomSheetRef<VisitorInfoComponent>,
    private fb: FormBuilder /* , private dataStore: AngularFirestore */
  ) {}
  ...
  ngOnInit() {
    ...
    this.visitorForm.get("country").valueChanges.subscribe((val: string) => {
      /* this.filteredCountriesList = observableOf(
        this.countriesList
          .filter((countryData) => {
            return (
